...
static bool powNative(int argCount, Value* args, Value* result) {
  if (!IS_NUMBER(args[0]) || !IS_NUMBER(args[1])) {
    runtimeError("Arguments to pow() must be numbers.");
    return false;
  }
...
static bool hasFieldNative(int argCount, Value* args, Value* result) {
  ...
  if (!IS_STRING(args[1])) {
    runtimeError("Second argument to hasField() must be a string.");
    return false;
  }
  ...
  *result = BOOL_VAL(
      tableGet(&instance->fields, AS_STRING(args[1]), &dummy));
...
static bool getFieldNative(int argCount, Value* args, Value* result) {
  ...
  if (!tableGet(&instance->fields, AS_STRING(args[1]), result)) {
    runtimeError("Undefined field '%s'.", AS_CSTRING(args[1]));
    return false;
  }
...
case OBJ_NATIVE: {
  ...
  Value result;
  if (!nativeObj->function(argCount, vm.stackTop - argCount, &result)) {
    return false;
  }
