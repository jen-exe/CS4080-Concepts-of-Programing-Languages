...
private final Stack<Map<String, Boolean>> used = new Stack<>();
...
public Void visitVariableExpr(Expr.Variable expr) {
  ...
  // CH11 Q3: mark variable as used
  if (!used.isEmpty()) {
    used.peek().put(expr.name.lexeme, true);
  }
...
//> begin-scope
 private void beginScope() {
  ...
  used.push(new HashMap<String, Boolean>());
  scopeNextIndex.push(0); // initialize index for this scope
}
//< begin-scope
//> end-scope
  private void endScope() {
    Map<String, Boolean> scope = used.pop();

    for (Map.Entry<String, Boolean> entry : scope.entrySet()) {
        if (!entry.getValue()) {
            Lox.error(0, "Variable '" + entry.getKey() + "' declared but never used.");
        }
    }
